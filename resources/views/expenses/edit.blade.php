                    ...
                    <div class="mb-3">
                        ...
                        <select name="project_id" class="form-select select-search @error('project_id') is-invalid @enderror">
                            ...
                            @foreach($projects as $p)
                                <option value="{{ $p->id }}" {{ old('project_id', $expense->project_id) == $p->id ? 'selected' : '' }}>{{ $p->name }} ({{ $p->client?->company_name ?? 'Internal Project' }})</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="payment_mode" class="form-select @error('payment_mode') is-invalid @enderror" required>
                            ...
                            @foreach($banks as $bank)
                                <option value="{{ $bank->name }}" {{ old('payment_mode', $expense->payment_mode) === $bank->name ? 'selected' : '' }}>{{ $bank->name }} - {{ $bank->branch }} - ****{{ substr($bank->account_number, -4) }}</option>
                            @endforeach
                        </select>
