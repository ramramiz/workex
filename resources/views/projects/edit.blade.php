                ...
                <form method="POST" action="{{ route('projects.update', $project) }}" enctype="multipart/form-data">
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-12 @if(auth()->user()->isSuperAdmin() || auth()->user()->isAccounts()) col-md-3 @else col-md-4 @endif">
                            ...
                            <select id="amc_service_type_select" name="amc_service_type" class="form-select @error('amc_service_type') is-invalid @enderror" required onchange="toggleCustomServiceType(this, 'edit_custom_service_type')">
                                <option value="AMC" {{ old('amc_service_type', $amc?->service_type ?? 'AMC') === 'AMC' ? 'selected' : '' }}>AMC</option>
                                <option value="Domain" {{ old('amc_service_type', $amc?->service_type) === 'Domain' ? 'selected' : '' }}>Domain</option>
                                <option value="Server" {{ old('amc_service_type', $amc?->service_type) === 'Server' ? 'selected' : '' }}>Server</option>
                                <option value="Email" {{ old('amc_service_type', $amc?->service_type) === 'Email' ? 'selected' : '' }}>Email</option>
                                <option value="other" {{ (old('amc_service_type') && !in_array(old('amc_service_type'), ['AMC', 'Domain', 'Server', 'Email'])) || ($amc && !in_array($amc->service_type, ['AMC', 'Domain', 'Server', 'Email'])) ? 'selected' : '' }}>Other (Custom)</option>
                            </select>
                            @php
                                $isCustom = (old('amc_service_type') && !in_array(old('amc_service_type'), ['AMC', 'Domain', 'Server', 'Email'])) || ($amc && !in_array($amc->service_type, ['AMC', 'Domain', 'Server', 'Email']));
                                $customVal = old('amc_service_type', $amc?->service_type);
                            @endphp
                            <input type="text" id="edit_custom_service_type" class="form-control mt-2 @if($isCustom) @else d-none @endif" placeholder="Enter custom service type" value="{{ $isCustom ? $customVal : '' }}" style="border-radius:8px;">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (serviceSelect && serviceSelect.value === 'other') {
        const customInput = document.getElementById('edit_custom_service_type');
