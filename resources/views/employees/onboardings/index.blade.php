...
@section('title', 'Employee Onboarding Links')
    ...
    <div class="col-12">
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
            <div class="btn-group shadow-sm">
                ...
                <a href="{{ route('employees.onboardings.index') }}" class="btn btn-primary btn-sm active">
    ...
    <div class="col-12">
        <div class="card shadow-sm border-0">
            <div class="card-header bg-white py-3">
                <h5 class="mb-0 fw-bold text-dark">Employee Onboarding Registrations</h5>
            </div>
            
            <!-- Filters -->
            <div class="card-body border-bottom py-3" style="background: var(--body-bg);">
                <form method="GET" action="{{ route('employees.onboardings.index') }}" class="row g-3">
                    ...
                    <div class="col-12 col-md-3 d-flex gap-2">
                        ...
                        @if(request()->filled('search') || request()->filled('status'))
                            <a href="{{ route('employees.onboardings.index') }}" class="btn btn-outline-secondary btn-sm px-3">Reset</a>
                        @endif
                    </div>
                </form>
            </div>
                            ...
                            <tr>
                                <td class="ps-4">
                                    <div class="d-flex align-items-center gap-3">
                                        <img src="https://ui-avatars.com/api/?name={{ urlencode($ob->name) }}&background=6366f1&color=fff" class="avatar-circle" style="width: 40px; height: 40px; border-radius: 50%;">
                                        <div>
                                            <div class="fw-semibold text-dark">{{ $ob->name }}</div>
                                            <div class="text-secondary style-subtext" style="font-size: 12.5px;">{{ $ob->email }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <div class="fw-semibold text-dark">{{ $ob->department->name ?? 'N/A' }}</div>
                                    <div class="text-secondary style-subtext" style="font-size: 12.5px;">{{ $ob->designation->name ?? 'N/A' }} • {{ $ob->sector ?? 'N/A' }}</div>
                                </td>
                                <td>
                                ...
                                <td>
                                    @if($ob->status !== 'completed')
                                        <div class="input-group input-group-sm" style="max-width: 320px;">
                                            <input type="text" class="form-control bg-light" id="link-{{ $ob->id }}" value="{{ route('employees.onboard.show', $ob->token) }}" readonly style="font-size: 11px;">
                                ...
                                <td class="pe-4 text-end">
                                    <div class="d-inline-flex gap-2">
                                        @if($ob->status === 'pending')
                                            ...
                                            <form action="{{ route('employees.onboardings.send-email', $ob->id) }}" method="POST" class="d-inline">
                                                @csrf
                                        ...
                                        @elseif($ob->status === 'submitted')
                                            <!-- Review Submission -->
                                            <a href="{{ route('employees.onboardings.review', $ob->id) }}" class="btn btn-sm btn-warning d-inline-flex align-items-center gap-1.5 text-dark fw-medium shadow-sm">
                                                <i class="bi bi-clipboard2-check-fill"></i> Review & Approve
                                            </a>
                                        @elseif($ob->status === 'completed')
                                            <!-- View Form Details -->
                                            <a href="{{ route('employees.onboardings.view', $ob->id) }}" class="btn btn-sm btn-outline-success d-inline-flex align-items-center gap-1.5" title="Print/View completed form details">
                                                <i class="bi bi-printer-fill"></i> Print Form
                                            </a>

                                            <!-- Edit Onboarding Details -->
                                            <a href="{{ route('employees.onboardings.edit', $ob->id) }}" class="btn btn-sm btn-outline-secondary d-inline-flex align-items-center gap-1.5" title="Edit onboarding details">
                                                <i class="bi bi-pencil-square"></i> Edit
                                            </a>
                                        @endif

                                        @if(auth()->user()->isAdminOrAbove() || auth()->user()->isHR())
                                            <form action="{{ route('employees.onboardings.destroy', $ob->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this onboarding record?');">
                                                @csrf
                                                @method('DELETE')
...
<div class="modal fade" id="generateLinkModal" tabindex="-1" aria-labelledby="generateLinkModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            ...
            <form method="POST" action="{{ route('employees.onboardings.generate') }}">
                @csrf
                <div class="modal-body p-4">
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            <label class="form-label fw-medium">Email Address (Personal) <span class="text-danger">*</span></label>
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <select name="department_id" id="modal_department_id" class="form-select" onchange="filterModalDesignations()" required>
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <select name="designation_id" id="modal_designation_id" class="form-select">
                        ...
                        <div class="col-12 col-md-6">
                            <label class="form-label fw-medium">Reporting Manager</label>
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <select name="role_id" class="form-select" required>
                                ...
                                @foreach($roles as $role)
                                    <option value="{{ $role->id }}" {{ $role->slug === 'employee' ? 'selected' : '' }}>{{ $role->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-12 col-md-6">
                            <label class="form-label fw-medium">Salary / Stipend <span class="text-danger">*</span></label>
                            <div class="input-group">
                                <span class="input-group-text">₹</span>
                                <input type="number" name="salary" class="form-control" placeholder="e.g. 25000" min="0" required>
                            </div>
                        </div>
                        <div class="col-12">
