...
@section('title', 'Register Vacancy')
@section('page-title', 'Register Vacancy')

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('job-vacancies.index') }}">Hiring & Vacancies</a></li>
    <li class="breadcrumb-item active">Register Vacancy</li>
@endsection
    ...
    <div class="col-12 col-lg-10 mx-auto">
        <div class="card shadow-sm border-0">
            <div class="card-header bg-white py-3">
                <h5 class="mb-0 fw-bold text-dark">Register New Job Vacancy</h5>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('job-vacancies.store') }}">
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-12 col-md-4">
                            ...
                            <input type="text" name="location" class="form-control @error('location') is-invalid @enderror" value="{{ old('location', 'Office') }}" required>
                        ...
                        <div class="col-12 col-md-4">
                            ...
                            <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                                <option value="active" {{ old('status') == 'active' ? 'selected' : '' }}>Active (Visible to candidates)</option>
                                <option value="inactive" {{ old('status') == 'inactive' ? 'selected' : '' }}>Inactive (Closed/Hidden)</option>
                            </select>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="requirements" class="form-control @error('requirements') is-invalid @enderror" rows="5">{{ old('requirements') }}</textarea>
                        @error('requirements')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="d-flex align-items-center justify-content-end gap-2 border-top pt-3">
                        <a href="{{ route('job-vacancies.index') }}" class="btn btn-outline-secondary">Cancel</a>
                        <button type="submit" class="btn btn-primary px-4">Register Vacancy</button>
                    </div>
                </form>
            </div>
        </div>
