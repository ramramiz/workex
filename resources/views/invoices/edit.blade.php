                ...
                <form method="POST" action="{{ route('invoices.update', $invoice) }}" id="invoiceForm">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-12 col-md-4">
                            ...
                            <input type="text" name="invoice_number" class="form-control fw-semibold" value="{{ old('invoice_number', $invoice->invoice_number) }}" required readonly>
                        </div>
                        <div class="col-12 col-md-4">
                            <label class="form-label">Invoice Date <span class="text-danger">*</span></label>
                            <input type="date" name="invoice_date" class="form-control @error('invoice_date') is-invalid @enderror" value="{{ old('invoice_date', $invoice->invoice_date ? $invoice->invoice_date->format('Y-m-d') : '') }}" required>
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <select name="project_id" id="project_id" class="form-select select-search @error('project_id') is-invalid @enderror" onchange="autoselectClient()">
                                ...
                                @foreach($projects as $p)
                                    <option value="{{ $p->id }}" data-client-id="{{ $p->client_id }}" {{ old('project_id', $invoice->project_id) == $p->id ? 'selected' : '' }}>{{ $p->name }} ({{ $p->client?->company_name ?? 'Internal Project' }})</option>
                                @endforeach
                            </select>
                    ...
                    <h6 class="text-uppercase text-primary fs-7 mb-3 border-bottom pb-2">Invoicing Calculations</h6>
                        ...
                        <div class="col-12 col-md-5">
                            <div class="card bg-light p-3">
                                <div class="mb-2 row align-items-center">
                                    ...
                                    <div class="col-sm-7">
                                        <input type="number" step="0.01" name="subtotal" id="subtotal" class="form-control form-control-sm fw-semibold" value="{{ $invoice->subtotal }}" readonly>
                                    </div>
                                </div>
                                <div class="mb-2 row align-items-center">
                                    <label class="col-sm-5 col-form-label col-form-label-sm fw-medium">Discount (₹)</label>
                                    <div class="col-sm-7">
                                        <input type="number" step="0.01" name="discount" id="discount" class="form-control form-control-sm" value="{{ $invoice->discount }}" oninput="calculateTotals()">
                                    </div>
                                </div>
                                <div class="mb-2 row align-items-center">
                                    <label class="col-sm-5 col-form-label col-form-label-sm fw-medium">GST (%)</label>
                                    <div class="col-sm-7">
                                        <input type="number" step="0.1" name="tax_percentage" id="tax_percentage" class="form-control form-control-sm" value="{{ $invoice->tax_percentage }}" oninput="calculateTotals()">
                                    </div>
                                </div>
                                <div class="mb-2 row align-items-center">
                                    <label class="col-sm-5 col-form-label col-form-label-sm fw-medium">GST Amount (₹)</label>
                                    <div class="col-sm-7">
                                        <input type="number" step="0.01" name="tax_amount" id="tax_amount" class="form-control form-control-sm" value="{{ $invoice->tax_amount }}" readonly>
                                    </div>
                                </div>
                                <hr class="my-2">
                                <div class="row align-items-center">
                                    ...
                                    <div class="col-sm-7">
                                        <input type="number" step="0.01" name="total" id="total" class="form-control form-control-sm fw-bold text-success fs-6" value="{{ $invoice->total }}" readonly required>
                                    </div>
                                </div>
                            </div>
                        </div>
